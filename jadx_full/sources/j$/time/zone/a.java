...
public final class a implements Externalizable {
    ...
    public static ZoneOffset b(DataInput dataInput) throws IOException {
        ...
        return b == 127 ? ZoneOffset.J(dataInput.readInt()) : ZoneOffset.J(b * 900);
    }

    public static void c(long j, DataOutput dataOutput) throws IOException {
        if (j < -4575744000L || j >= 10413792000L || j % 900 != 0) {
            dataOutput.writeByte(DnsRecord.CLASS_ANY);
            dataOutput.writeLong(j);
        } else {
            int i = (int) ((j + 4575744000L) / 900);
            dataOutput.writeByte((i >>> 16) & DnsRecord.CLASS_ANY);
            dataOutput.writeByte((i >>> 8) & DnsRecord.CLASS_ANY);
            dataOutput.writeByte(i & DnsRecord.CLASS_ANY);
        }
    }
