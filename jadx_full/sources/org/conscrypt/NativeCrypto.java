...
public final class NativeCrypto {
    ...
    static {
        ...
        loadError = e;
        String[] strArr = {Ciphers.TLS_AES_128_GCM_SHA256, Ciphers.TLS_AES_256_GCM_SHA384, Ciphers.TLS_CHACHA20_POLY1305_SHA256};
        ...
        if (e == null) {
            ...
            if (length % 2 != 0) {
                xe.k("Invalid cipher list returned by get_cipher_names");
                return;
            }
            int i = length / 2;
            SUPPORTED_TLS_1_2_CIPHER_SUITES = new String[i + 2];
            for (int i2 = 0; i2 < length; i2 += 2) {
                String strCipherSuiteToJava = cipherSuiteToJava(strArr2[i2]);
        ...
        DEFAULT_X509_CIPHER_SUITES = z ? new String[]{Ciphers.TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256, Ciphers.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA, Ciphers.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA, Ciphers.TLS_RSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_RSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_RSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_RSA_WITH_AES_256_CBC_SHA} : new String[]{Ciphers.TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256, Ciphers.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA, Ciphers.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA, Ciphers.TLS_RSA_WITH_AES_128_GCM_SHA256, Ciphers.TLS_RSA_WITH_AES_256_GCM_SHA384, Ciphers.TLS_RSA_WITH_AES_128_CBC_SHA, Ciphers.TLS_RSA_WITH_AES_256_CBC_SHA};
        DEFAULT_PSK_CIPHER_SUITES = new String[]{Ciphers.TLS_ECDHE_PSK_WITH_CHACHA20_POLY1305_SHA256, Ciphers.TLS_ECDHE_PSK_WITH_AES_128_CBC_SHA, Ciphers.TLS_ECDHE_PSK_WITH_AES_256_CBC_SHA, Ciphers.TLS_PSK_WITH_AES_128_CBC_SHA, Ciphers.TLS_PSK_WITH_AES_256_CBC_SHA};
        ...
        SUPPORTED_PROTOCOLS = new String[]{"TLSv1", "TLSv1.1", "TLSv1.2", "TLSv1.3"};
    }
    ...
    private static int X509_NAME_hash(X500Principal x500Principal, String str) {
        try {
            byte[] bArrDigest = MessageDigest.getInstance(str).digest(x500Principal.getEncoded());
    ...
    public static String[] checkEnabledCipherSuites(String[] strArr) {
        if (strArr == null) {
            xe.k("cipherSuites == null");
            return null;
        }
        for (int i = 0; i < strArr.length; i++) {
            String str = strArr[i];
            if (str == null) {
                xe.k(ha0.k("cipherSuites[", i, "] == null"));
                return null;
            }
            if (!str.equals(TLS_EMPTY_RENEGOTIATION_INFO_SCSV) && !strArr[i].equals(TLS_FALLBACK_SCSV) && !SUPPORTED_TLS_1_2_CIPHER_SUITES_SET.contains(strArr[i]) && !SUPPORTED_LEGACY_CIPHER_SUITES_SET.contains(strArr[i])) {
                xe.k(fw.y(new StringBuilder("cipherSuite "), strArr[i], " is not supported."));
                return null;
            }
        }
        return strArr;
    ...
    private static int getProtocolConstant(String str) {
        ...
        xe.i("Unknown protocol encountered: ".concat(str));
        return 0;
    }
