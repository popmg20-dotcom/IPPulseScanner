...
public class DnsRecordType implements Comparable<DnsRecordType> {
    ...
    static {
        ...
        DnsRecordType dnsRecordType37 = new DnsRecordType(DnsRecord.CLASS_ANY, "ANY");
        ANY = dnsRecordType37;
        ...
        StringBuilder sbB = fw.B(512, " (expected: ");
        for (int i = 0; i < 40; i++) {
            DnsRecordType dnsRecordType41 = dnsRecordTypeArr[i];
            BY_NAME.put(dnsRecordType41.name(), dnsRecordType41);
            BY_TYPE.put(dnsRecordType41.intValue(), dnsRecordType41);
            sbB.append(dnsRecordType41.name());
            sbB.append('(');
            sbB.append(dnsRecordType41.intValue());
            sbB.append("), ");
        }
        sbB.setLength(sbB.length() - 2);
        sbB.append(')');
        EXPECTED = sbB.toString();
    }

    public DnsRecordType(int i, String str) {
        if ((65535 & i) != i) {
            xe.k(ha0.k("intValue: ", i, " (expected: 0 ~ 65535)"));
            throw null;
        }
        this.intValue = i;
        this.name = str;
