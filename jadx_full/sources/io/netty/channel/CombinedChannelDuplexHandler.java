...
public class CombinedChannelDuplexHandler<I extends ChannelInboundHandler, O extends ChannelOutboundHandler> extends ChannelDuplexHandler {
    ...
    private void checkAdded() {
        ...
        xe.q("handler not added to pipeline yet");
    }

    private void validate(I i, O o) {
        if (this.inboundHandler != null) {
            xe.q("init() can not be invoked if CombinedChannelDuplexHandler was constructed with non-default constructor.");
            return;
        }
        ObjectUtil.checkNotNull(i, "inboundHandler");
        ObjectUtil.checkNotNull(o, "outboundHandler");
        if (i instanceof ChannelOutboundHandler) {
            xe.k("inboundHandler must not implement ChannelOutboundHandler to get combined.");
        } else if (o instanceof ChannelInboundHandler) {
            xe.k("outboundHandler must not implement ChannelInboundHandler to get combined.");
        }
    }
    ...
    public void handlerAdded(ChannelHandlerContext channelHandlerContext) {
        if (this.inboundHandler == null) {
            xe.q("init() must be invoked before being added to a ChannelPipeline if CombinedChannelDuplexHandler was constructed with the default constructor.");
            return;
        }
    ...
    public static class DelegatingChannelHandlerContext implements ChannelHandlerContext {
        ...
        public void remove0() {
            ...
            try {
                ...
                fireExceptionCaught((Throwable) new ChannelPipelineException(this.handler.getClass().getName().concat(".handlerRemoved() has thrown an exception."), th));
            }
        }
