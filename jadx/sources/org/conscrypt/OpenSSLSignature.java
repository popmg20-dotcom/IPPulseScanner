...
public class OpenSSLSignature extends SignatureSpi {
    ...
    /* JADX INFO: renamed from: org.conscrypt.OpenSSLSignature$1, reason: invalid class name */
    /* JADX INFO: compiled from: r8-map-id-605be6ced3201ebf27d05845df89146b03500986a7a8ae40fc58e667db41f02f */
    ...
    public static abstract class RSAPSSPadding extends OpenSSLSignature {
        ...
        public final void engineSetParameter(AlgorithmParameterSpec algorithmParameterSpec) throws InvalidAlgorithmParameterException {
            ...
            if (!"MGF1".equalsIgnoreCase(mGFAlgorithm) && !"1.2.840.113549.1.1.8".equals(mGFAlgorithm)) {
                throw new InvalidAlgorithmParameterException(ha0.o("Unsupported MGF algorithm: ", mGFAlgorithm, ". Only MGF1 supported"));
            }
            ...
            try {
                ...
                if (saltLength < 0) {
                    throw new InvalidAlgorithmParameterException(dw2.A(saltLength, "Salt length must be non-negative: "));
                }
                int trailerField = pSSParameterSpec.getTrailerField();
                if (trailerField != 1) {
                    throw new InvalidAlgorithmParameterException(ha0.k("Unsupported trailer field: ", trailerField, ". Only 1 supported"));
                }
    ...
    private void resetContext() {
        ...
        if (z) {
            jEVP_DigestVerifyInit = NativeCrypto.EVP_DigestSignInit(evp_md_ctx, j, this.key.getNativeRef());
            this.evpPkeyCtx = jEVP_DigestVerifyInit;
        } else {
            jEVP_DigestVerifyInit = NativeCrypto.EVP_DigestVerifyInit(evp_md_ctx, j, this.key.getNativeRef());
            this.evpPkeyCtx = jEVP_DigestVerifyInit;
        }
        configureEVP_PKEY_CTX(jEVP_DigestVerifyInit);
        this.ctx = evp_md_ctx;
    }
    ...
    public byte[] engineSign() {
        try {
            ...
            xe.i("Reset of context failed after it was successful once");
            return null;
        }
    }
    ...
    public void engineUpdate(ByteBuffer byteBuffer) {
        if (byteBuffer.hasRemaining()) {
            ...
            if (this.signing) {
                NativeCrypto.EVP_DigestSignUpdateDirect(evp_md_ctx, j, iRemaining);
            } else {
                NativeCrypto.EVP_DigestVerifyUpdateDirect(evp_md_ctx, j, iRemaining);
            }
    ...
    public boolean engineVerify(byte[] bArr) {
        try {
            try {
                boolean zEVP_DigestVerifyFinal = NativeCrypto.EVP_DigestVerifyFinal(this.ctx, bArr, 0, bArr.length);
                try {
                    resetContext();
