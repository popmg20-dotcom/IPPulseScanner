...
final class OpenSSLECGroupContext {
    ...
    public static OpenSSLECGroupContext getCurveByName(String str) {
        ...
        long jEC_GROUP_new_by_curve_name = NativeCrypto.EC_GROUP_new_by_curve_name(str);
        if (jEC_GROUP_new_by_curve_name == 0) {
            return null;
        }
        return new OpenSSLECGroupContext(new NativeRef.EC_GROUP(jEC_GROUP_new_by_curve_name));
    }
    ...
    public static OpenSSLECGroupContext getInstance(ECParameterSpec eCParameterSpec) {
        ...
        try {
            long jEC_GROUP_new_arbitrary = NativeCrypto.EC_GROUP_new_arbitrary(p.toByteArray(), a.toByteArray(), b.toByteArray(), affineX.toByteArray(), affineY.toByteArray(), order.toByteArray(), eCParameterSpec.getCofactor());
            if (jEC_GROUP_new_arbitrary != 0) {
                return new OpenSSLECGroupContext(new NativeRef.EC_GROUP(jEC_GROUP_new_arbitrary));
            }
    ...
    public ECParameterSpec getECParameterSpec() {
        String strEC_GROUP_get_curve_name = NativeCrypto.EC_GROUP_get_curve_name(this.groupCtx);
        byte[][] bArrEC_GROUP_get_curve = NativeCrypto.EC_GROUP_get_curve(this.groupCtx);
        BigInteger bigInteger = new BigInteger(bArrEC_GROUP_get_curve[0]);
        ECParameterSpec eCParameterSpec = new ECParameterSpec(new EllipticCurve(new ECFieldFp(bigInteger), new BigInteger(bArrEC_GROUP_get_curve[1]), new BigInteger(bArrEC_GROUP_get_curve[2])), new OpenSSLECPointContext(this, new NativeRef.EC_POINT(NativeCrypto.EC_GROUP_get_generator(this.groupCtx))).getECPoint(), new BigInteger(NativeCrypto.EC_GROUP_get_order(this.groupCtx)), new BigInteger(NativeCrypto.EC_GROUP_get_cofactor(this.groupCtx)).intValue());
        Platform.setCurveName(eCParameterSpec, strEC_GROUP_get_curve_name);
