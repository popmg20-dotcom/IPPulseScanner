...
public final class Base64 {
    ...
    public static final class Decoder implements ByteProcessor {
        ...
        private static int decode4to3(byte[] bArr, ByteBuf byteBuf, int i, byte[] bArr2) {
            ...
            if (b3 == 61) {
                try {
                    byteBuf.setByte(i, ((bArr2[b] & Base64.EQUALS_SIGN_ENC) << 2) | ((bArr2[b2] & Base64.EQUALS_SIGN_ENC) >>> 4));
                    return 1;
                } catch (IndexOutOfBoundsException unused) {
                    xe.k("not encoded in Base64");
                    return 0;
                }
            }
            byte b4 = bArr[3];
            ...
            try {
                if (byteBuf.order() == ByteOrder.BIG_ENDIAN) {
                    i2 = ((bArr2[b] & 63) << 18) | ((bArr2[b2] & Base64.EQUALS_SIGN_ENC) << 12) | ((bArr2[b3] & Base64.EQUALS_SIGN_ENC) << 6);
                    i3 = bArr2[b4] & Base64.EQUALS_SIGN_ENC;
                } else {
                    byte b6 = bArr2[b2];
                    byte b7 = bArr2[b3];
                    i2 = ((bArr2[b] & 63) << 2) | ((b6 & 15) << 12) | ((b6 & 240) >>> 4) | ((b7 & 3) << 22) | ((b7 & 252) << 6);
                    i3 = (bArr2[b4] & Base64.EQUALS_SIGN_ENC) << 16;
                }
        ...
        public ByteBuf decode(ByteBuf byteBuf, int i, int i2, ByteBufAllocator byteBufAllocator, Base64Dialect base64Dialect) throws Throwable {
        ...
        public boolean process(byte b) {
            ...
            if (b <= 0 || (b2 = (bArr = this.decodabet)[b]) < -5) {
                xe.k(fw.w(" (decimal)", new StringBuilder("invalid Base64 input character: "), (short) (b & Base64.EQUALS_SIGN_ENC)));
                return false;
            }
            if (b2 >= -1) {
                byte[] bArr2 = this.b4;
    ...
    public static ByteBuf encode(ByteBuf byteBuf, int i, int i2, boolean z, Base64Dialect base64Dialect, ByteBufAllocator byteBufAllocator) {
        ObjectUtil.checkNotNull(byteBuf, "src");
        ObjectUtil.checkNotNull(base64Dialect, "dialect");
        ByteBuf byteBufOrder = byteBufAllocator.buffer(encodedBufferSize(i2, z)).order(byteBuf.order());
        ...
        while (i4 < i3) {
            ...
            encode3to4(byteBuf2, i4 + i, 3, byteBufOrder, i5, bArrAlphabet);
            i6 += 4;
            if (z && i6 == MAX_LINE_LENGTH) {
                byteBufOrder.setByte(i5 + 4, 10);
                i5++;
                i6 = 0;
            }
            i4 += 3;
        ...
        if (i4 < i2) {
            encode3to4(byteBuf3, i4 + i, i2 - i4, byteBufOrder, i5, bArrAlphabet);
            i5 += 4;
        }
        if (i5 > 1 && byteBufOrder.getByte(i5 - 1) == 10) {
            i5--;
        }
        return byteBufOrder.slice(0, i5);
    }
    ...
    private static void encode3to4(ByteBuf byteBuf, int i, int i2, ByteBuf byteBuf2, int i3, byte[] bArr) {
        ...
        if (byteBuf.order() == ByteOrder.BIG_ENDIAN) {
            if (i2 == 1) {
                ...
                intLE = toIntBE(byteBuf.getShort(i));
            } else if (i2 > 0) {
                intLE = toIntBE(byteBuf.getMedium(i));
            }
            encode3to4BigEndian(intLE, i2, byteBuf2, i3, bArr);
            return;
        }
        if (i2 == 1) {
            intLE = toInt(byteBuf.getByte(i));
            ...
            intLE = toIntLE(byteBuf.getMedium(i));
        }
    ...
    public static int encodedBufferSize(int i, boolean z) {
        ...
        return j2 < 2147483647L ? (int) j2 : ChannelUtils.WRITE_STATUS_SNDBUF_FULL;
    }

    private static int toInt(byte b) {
        return (b & EQUALS_SIGN_ENC) << 16;
    }
    ...
    private static int toIntLE(int i) {
        return ((i & 16711680) >>> 16) | ((i & DnsRecord.CLASS_ANY) << 16) | (65280 & i);
    }
