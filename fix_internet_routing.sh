...
public class GamingVpnService extends VpnService {
    ...
    // Standard VPN TUN interface settings
    ...
    private void startForegroundCompatible() {
        Notification notification = buildNotification("VPN Active (MTU=" + currentMtu + ")");
        try {
    ...
    private void startVpn() {
        try {
            ...
            // Use public reliable DNS for proper name resolution
            ...
            Log.i(TAG, "VPN started successfully with proper routes.");
        } catch (Exception e) {
